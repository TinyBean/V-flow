...
# 优先 NVENC 硬件编码(快 ~50x),由 VIDEO_ENC 决定(libx264 / h264_nvenc)。
transcode() {
    if [ "${VIDEO_ENC:-libx264}" = h264_nvenc ]; then
        ffmpeg_run "$1" "$2" -c:v h264_nvenc -preset p5 -tune hq -rc vbr -cq 20 -c:a aac
    else
        ffmpeg_run "$1" "$2" -c:v libx264 -crf 20 -preset medium -c:a aac
    fi
}

# 探测可用视频编码器:优先 h264_nvenc(需编译进 ffmpeg 且 GPU 可用),否则 libx264。
# 注意:探测尺寸必须 ≥ NVENC 最小支持尺寸(16x16 会被拒绝),用 160x120。
detect_encoder() {
    ffmpeg -hide_banner -encoders 2>/dev/null | grep -qw h264_nvenc || { echo libx264; return; }
    ffmpeg -y -f lavfi -i color=black:s=160x120:d=0.04 -frames:v 1 -c:v h264_nvenc -f null - \
        >/dev/null 2>&1 && echo h264_nvenc || echo libx264
}
...
[ "$VIDEO_ENC" = h264_nvenc ] \
    && echo "转码编码器: h264_nvenc(NVIDIA 硬件,高速)" \
    || echo "转码编码器: libx264(软件)"
...
export FFMPEG_TIMEOUT TS_EXTS FS_EXTS VIDEO_ENC
