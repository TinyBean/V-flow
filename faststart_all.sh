...
classify() {
    ...
    case " $FS_EXTS " in *" $ext "*) needs_faststart "$1" && echo faststart || echo ok; return ;; esac
    case " $TS_EXTS " in *" $ext "*) echo ts; return ;; esac
    echo none
}

# 无损 -c copy + faststart 重封装 src -> dst;先写 .part 再 mv。成功返回 0。
remux() {
    local src="$1" dst="$2" tmp="${2}.part"
    rm -f -- "$tmp"
    local cmd=(ffmpeg -y -i "$src" -c copy -movflags +faststart -f "$(container_fmt "$dst")" "$tmp")
    local rc=0
...
selftest() {
    ...
    local d a b ok=1
    d=$(mktemp -d); a="$d/a.mp4"; b="$d/b.mp4"
    ...
    [ -s "$a" ] && [ -s "$b" ] || { echo "[selftest] 生成测试 mp4 失败"; rm -rf -- "$d"; exit 1; }
...
printf '目录: %s\n' "$DIR"
printf '待处理: %d 个 .ts(转 MP4 + 删原文件) | %d 个 MP4(moov 前置)\n' "$ts_n" "$fs_n"
if [ "$((ts_n + fs_n))" -eq 0 ]; then
    echo "没有需要处理的文件(.ts 全已转、MP4 全已 faststart)。"; exit 0
fi
...
export -f lower_ext container_fmt needs_faststart classify remux process_file
tmplog=$(mktemp)
printf '%s\0' "${items[@]}" \
